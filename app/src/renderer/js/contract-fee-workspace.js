...
(function contractFeeWorkspace(root) {
  ...
  function templateResidentSyncModal(batch, completeAfter = false) {
    if (!batch) throw new Error('未找到发放批次'); const plan = model.disbursementResidentImportPlan(batch, state.database.personnel);
    state.modal = { type: 'template-resident-sync', batchId: batch.id, completeAfter };
    const rows = plan.map((entry) => { const person = entry.person; const choices = (entry.candidates || []).map((candidate) => `<option value="${escapeHtml(candidate.personId)}">${escapeHtml(model.personName(candidate.person))} · ${escapeHtml(model.personGroup(candidate.person) || '未分组')}</option>`).join(''); const action = entry.status === 'manual' ? `<select name="personId"><option value="">请选择居民</option>${choices}</select><select name="conflictResolution"><option value="keep">保留居民资料</option><option value="adopt">采用本次资料</option></select>` : (entry.status === 'create' ? '将新建居民档案' : `已关联：${escapeHtml(model.personName(person) || '')}`); return `<tr data-template-resident-sync="${escapeHtml(entry.itemId)}"><td><strong>${escapeHtml(entry.item.name)}</strong><br><span class="text-secondary">${escapeHtml(entry.item.groupName || '未分组')}</span></td><td>${escapeHtml(entry.item.idCard || '未填写')}<br>${escapeHtml(entry.item.phone || '—')}</td><td>${escapeHtml(entry.item.bankCard || '—')}<br>${escapeHtml(entry.item.bankName || '—')}</td><td>${escapeHtml(entry.reason)}</td><td>${action}</td></tr>`; }).join('');
    openModal('同步居民资料预览', `<div class="cf-hint">先在这里核对姓名、身份证、手机号、银行卡与开户行。只有点击确认后才会写入居民档案；不一致的资料需明确选择保留原资料或采用本次资料。</div><div class="cf-table-wrap"><table class="cf-table"><thead><tr><th>本次收款人</th><th>证件 / 手机号</th><th>银行卡 / 开户行</th><th>处理说明</th><th>确认方式</th></tr></thead><tbody>${rows}</tbody></table></div>`, { footer: `<button class="btn btn-outline" data-cf-action="close-modal">取消</button><button class="btn btn-primary" data-cf-action="confirm-template-resident-sync">${completeAfter ? '确认同步并发放完成' : '确认同步到居民档案'}</button>` });
  }
  async function completeTemplateBatch(id) { const current = findById('disbursementBatches', id); if (!current) throw new Error('未找到发放批次'); const sync = model.disbursementBatchSyncStatus(current, state.database.personnel); if (sync.code !== 'synced') return templateResidentSyncModal(current, true); const resolutions = current.residentSyncDecisions || Object.fromEntries((current.items || []).filter((item) => item.personId).map((item) => [item.id, { personId: item.personId, bankCardDecision: 'sync' }])); const result = model.completeTemplateDisbursementBatch(current, { personnel: state.database.personnel, resolutions }); state.database.personnel = result.personnel; state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1, result.batch); await saveDatabase('已完成发放并同步居民资料'); closeModal(); renderShell(); }
  async function confirmTemplateResidentSync() {
    ...
    document.querySelectorAll('[data-template-resident-sync]').forEach((row) => { const personId = row.querySelector('[name="personId"]')?.value; const conflictResolution = row.querySelector('[name="conflictResolution"]')?.value; if (personId || conflictResolution) resolutions[row.dataset.templateResidentSync] = { personId, conflictResolution }; });
    const syncResult = model.syncDisbursementBatchResidents({ batch: current, personnel: state.database.personnel, resolutions }); state.database.personnel = syncResult.personnel; state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1, syncResult.batch); await saveDatabase(syncResult.summary.manual ? `已同步 ${syncResult.summary.created + syncResult.summary.merged} 条资料，仍有 ${syncResult.summary.manual} 条待人工处理` : '居民资料已同步');
    if (syncResult.summary.manual) return templateResidentSyncModal(syncResult.batch, completeAfter);
    if (completeAfter) return completeTemplateBatch(syncResult.batch.id);
    closeModal(); renderShell();
  }
