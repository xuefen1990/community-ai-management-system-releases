...
(function contractFeeWorkspace(root) {
  ...
  async function saveTemplateBatch() {
    ...
    try { const values = Object.fromEntries(new FormData(form).entries()); const template = templateFor(state.modal.templateId || state.modal.templateKey); const items = [...form.querySelectorAll('.cf-template-item')].map((row) => { const customData = Object.fromEntries([...row.querySelectorAll('[data-template-custom-field]')].map((input) => [input.dataset.templateCustomField, input.value])); return { personId: itemValue(row, 'personId'), name: itemValue(row, 'name'), idCard: row.dataset.importIdCard, groupName: row.dataset.importGroupName, role: itemValue(row, 'role'), responsibilityArea: itemValue(row, 'responsibilityArea'), workDate: itemValue(row, 'workDate'), workItem: itemValue(row, 'workItem'), quantity: itemValue(row, 'quantity'), unitPrice: itemValue(row, 'unitPrice'), deductions: itemValue(row, 'deductions'), bankCard: itemValue(row, 'bankCard'), finalAmount: itemValue(row, 'finalAmount'), adjustmentReason: itemValue(row, 'adjustmentReason'), remark: itemValue(row, 'remark'), customData }; }).filter((item) => item.personId || item.name);
      const existing = findById('disbursementBatches', state.modal?.batchId); const created = model.createTemplateDisbursementBatch({ ...values, templateId: template?.id, templateSnapshot: template, templateKey: state.modal.templateKey, items, printSettings: { paper: values.paper, rowsPerPage: values.rowsPerPage } }, { personnel: state.database.personnel, id: existing?.id }); const batch = existing ? { ...existing, ...created, id: existing.id, createdAt: existing.createdAt, status: 'draft', preparedAt: null, printedAt: null, reviewedAt: null, completedAt: null } : created;
  ...
  function templatePreviewHtml(batch) {
    ...
    const total = model.summarizeDisbursementBatch(batch).totalCents; const paper = text(batch.printSettings?.paper || template?.paper || (batch.templateKey === 'contract_fee' ? 'A4' : 'A5')).toLowerCase(); const orientation = template?.orientation === 'landscape' ? 'landscape' : 'portrait'; const perPage = Math.max(1, Number(batch.printSettings?.rowsPerPage || template?.rowsPerPage || (paper === 'a4' ? 20 : 10))); const pages = [];
    for (let index = 0; index < batch.items.length; index += perPage) pages.push(batch.items.slice(index, index + perPage));
    return pages.map((items, pageIndex) => { const rows = items.map((item, index) => `<tr>${rowCells(item, pageIndex * perPage + index).map((cell) => `<td>${escapeHtml(cell || '')}</td>`).join('')}</tr>`).join(''); const isLast = pageIndex === pages.length - 1; return `<article class="cf-print-sheet ${paper} ${orientation}" data-cf-print-page="${pageIndex + 1}"><h1>${escapeHtml(batch.title || template?.title || templateLabel(batch.templateKey))}</h1><div class="cf-print-meta"><span>编制单位：${escapeHtml(batch.villageName || '')}</span><span>期间：${escapeHtml(batch.period || '')}</span><span>日期：${escapeHtml(batch.batchDate || '')}</span><span>单位：元</span></div><table><thead><tr>${headers.map((header) => `<th>${escapeHtml(header)}</th>`).join('')}</tr></thead><tbody>${rows}${isLast ? `<tr><td colspan="${Math.max(1, headers.length - 3)}">合计</td><td colspan="3">${money(total)}　共 ${batch.items.length} 人</td></tr>` : ''}</tbody></table><div class="cf-print-signers"><span>审批人：${escapeHtml(batch.signers?.approver || '')}</span><span>制表人：${escapeHtml(batch.signers?.maker || '')}</span>${batch.templateKey === 'casual_labor' ? `<span>经办人：${escapeHtml(batch.signers?.handler || '')}</span>` : ''}<span>第 ${pageIndex + 1}/${pages.length} 页</span></div></article>`; }).join('');
  }

  function templatePreviewModal(batch) { const perPage = Math.max(1, Number(batch.printSettings?.rowsPerPage || 10)); const pages = Math.max(1, Math.ceil((batch.items || []).length / perPage)); const template = templateFor(batch.templateId || batch.templateKey, batch) || {}; state.modal = { type: 'template-preview', batchId: batch.id }; openModal('打印预览', `<div class="cf-preview-workspace"><aside class="cf-preview-settings"><h4>打印设置</h4><dl><div><dt>纸张</dt><dd>${escapeHtml(batch.printSettings?.paper || 'A5')}</dd></div><div><dt>方向</dt><dd>${template.orientation === 'landscape' ? '横向' : '纵向'}</dd></div><div><dt>每页人数</dt><dd>${perPage} 人</dd></div><div><dt>合计页数</dt><dd>${pages} 页</dd></div></dl><p>编辑发放表后，预览会按照当前数据自动分页。</p>${['draft', 'prepared'].includes(batch.status) ? `<button class="btn btn-outline" data-cf-action="edit-template-batch" data-id="${batch.id}">返回编辑表</button>` : ''}</aside><section class="cf-preview-canvas"><div class="cf-hint">这是打印成品预览，使用的是本批次保存的模板快照与明细数据。</div><div id="cf-template-preview">${templatePreviewHtml(batch)}</div></section><aside class="cf-preview-pages"><h4>页面缩略</h4>${Array.from({ length: pages }, (_value, index) => `<button class="cf-preview-page-button" data-cf-preview-page="${index + 1}"><strong>第 ${index + 1} 页</strong><span>${index === pages - 1 ? `含合计 · ${batch.items.length} 人` : `第 ${(index * perPage) + 1}–${Math.min((index + 1) * perPage, batch.items.length)} 人`}</span></button>`).join('')}</aside></div>`, { footer: `<button class="btn btn-outline" data-cf-action="close-modal">关闭</button>${batch.status === 'draft' ? `<button class="btn btn-outline" data-cf-action="prepare-template" data-id="${batch.id}">准备打印</button>` : ''}<button class="btn btn-outline" data-cf-action="export-template-batch" data-id="${batch.id}">导出 Excel</button><button class="btn btn-outline" data-cf-action="complete-template" data-id="${batch.id}">发放完成并同步</button><button class="btn btn-primary" data-cf-action="print-template">打印</button>` }); }

  async function printTemplate() { const source = document.getElementById('cf-template-preview')?.innerHTML; if (!source) return; const current = findById('disbursementBatches', state.modal?.batchId); const template = current ? templateFor(current.templateId || current.templateKey, current) : {}; const paper = text(current?.printSettings?.paper || template?.paper || 'A5').toUpperCase(); const pageSize = `${paper} ${template?.orientation === 'landscape' ? 'landscape' : 'portrait'}`; const popup = root.open('', '_blank'); if (!popup) throw new Error('未能打开打印窗口，请允许本软件打开打印预览'); popup.document.write(`<html><head><title>发放表打印</title><style>body{margin:0;font-family:"Songti SC",serif;color:#000}.cf-print-sheet{box-sizing:border-box;margin:0 auto;padding:12mm;page-break-after:always}.cf-print-sheet:last-child{page-break-after:auto}.cf-print-sheet.a5{width:148mm;min-height:210mm}.cf-print-sheet.a4{width:210mm;min-height:297mm}.cf-print-sheet.a5.landscape{width:210mm;min-height:148mm}.cf-print-sheet.a4.landscape{width:297mm;min-height:210mm}.cf-print-sheet h1{text-align:center;font-size:18pt;margin:0 0 8mm}.cf-print-meta,.cf-print-signers{display:flex;justify-content:space-between;gap:8px;margin:4mm 0;font-size:10pt}.cf-print-sheet table{border-collapse:collapse;width:100%;font-size:9pt}.cf-print-sheet th,.cf-print-sheet td{border:1px solid #000;padding:2.2mm 1.5mm;text-align:center;word-break:break-all}@page{size:${pageSize};margin:0}@media print{.cf-print-sheet{margin:0}}</style></head><body>${source}<script>window.onload=()=>window.print()<\/script></body></html>`); popup.document.close(); if (current && typeof model.markTemplateDisbursementPrinted === 'function') { const next = model.markTemplateDisbursementPrinted(current); state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1, next); await saveDatabase('已登记打印时间'); } }
