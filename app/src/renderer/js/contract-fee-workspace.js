...
(function contractFeeWorkspace(root) {
  ...
  function ensureCollections(database) {
    ...
    if (!Array.isArray(database.residentOperationLog)) database.residentOperationLog = [];
  ...
  function renderShell() {
    ...
    section.innerHTML = `
      ...
      <div class="cf-nav">${[['overview', '汇总看板'], ['general-batches', '全部发放批次'], ['recycle-bin', `资金回收站${state.database.disbursementRecycleBin.length ? `（${state.database.disbursementRecycleBin.length}）` : ''}`], ['profiles', '固定人员台账'], ['subsidies', '年度地力补贴'], ['ledger', '承包费历史台账'], ['batches', '历史承包费记录']].map(([key, label]) => `<button class="${state.view === key ? 'active' : ''}" data-cf-view="${key}">${label}</button>`).join('')}</div>
      <div id="cf-view"></div>`;
    renderView();
  }
  ...
  function renderView() {
    ...
    else if (state.view === 'recycle-bin') target.innerHTML = renderDisbursementRecycleBin();
  ...
  function batchMatchesQuery(batch, query) { const needle = text(query).toLowerCase(); return !needle || [batch.categoryName, batch.period, batch.batchDate, batch.notes, batch.templateSnapshot?.name, ...(batch.items || []).flatMap((item) => [item.name, item.groupName, item.idCard, item.bankCard])].some((value) => text(value).toLowerCase().includes(needle)); }
  function batchSyncBadge(batch) { const status = model.disbursementBatchSyncStatus(batch, state.database.personnel); return `<button class="cf-badge ${status.code === 'synced' ? 'ok' : 'warn'}" data-cf-action="preview-template-sync" data-id="${escapeHtml(batch.id)}">${escapeHtml(status.label)}</button>`; }
  function renderGeneralBatches() {
    const batches = [...state.database.disbursementBatches].filter((batch) => batchMatchesQuery(batch, state.disbursementQuery)).sort((a, b) => text(b.createdAt).localeCompare(text(a.createdAt)));
    const rows = batches.map((batch) => { const summary = model.summarizeDisbursementBatch(batch); const test = batch.isTest ? '<span class="cf-badge warn">测试批次</span>' : ''; return `<tr><td><strong>${escapeHtml(batch.categoryName || templateLabel(batch.templateKey))}</strong><br><span class="text-secondary">${escapeHtml(batch.period)} · ${escapeHtml(batch.batchDate || '未填日期')}</span><br>${test}</td><td>${statusBadge(batch.status)}<br>${batchSyncBadge(batch)}</td><td>${money(summary.totalCents)}</td><td>${summary.paidCount}/${summary.recipientCount} 人</td><td>${escapeHtml(batch.notes || '—')}</td><td><div class="cf-row-actions">${batch.status === 'draft' && !batch.templateKey ? `<button data-cf-action="review-disbursement" data-id="${batch.id}">审核</button>` : ''}${batch.status !== 'completed' && !batch.templateKey ? `<button data-cf-action="pay-disbursement" data-id="${batch.id}">发放并同步</button>` : ''}<button data-cf-action="copy-disbursement-batch" data-id="${batch.id}">复制</button><button data-cf-action="recycle-disbursement-batch" data-id="${batch.id}">删除</button><button data-cf-action="view-disbursement" data-id="${batch.id}">查看</button></div></td></tr>`; }).join('');
    return `<div class="cf-panel"><div class="cf-panel-head"><div><h3>全部发放批次</h3><span class="text-secondary">承包费、补贴、工资和自定义类别统一查询</span></div><div class="cf-row-actions"><input id="cf-disbursement-query" value="${escapeHtml(state.disbursementQuery)}" placeholder="姓名、身份证、卡号、组别、日期或模板"><button class="btn btn-outline" data-cf-action="query-disbursement-batches">查询</button><button class="btn btn-outline" data-cf-action="clear-disbursement-query">清除</button></div></div><div class="cf-table-wrap"><table class="cf-table"><thead><tr><th>类别 / 期间</th><th>状态 / 资料</th><th>金额</th><th>收款人</th><th>备注</th><th>操作</th></tr></thead><tbody>${rows || '<tr><td colspan="6"><div class="cf-empty">暂无符合条件的发放批次。</div></td></tr>'}</tbody></table></div></div>`;
  }
  function renderDisbursementRecycleBin() {
    const rows = [...state.database.disbursementRecycleBin].sort((a, b) => text(b.recycleInfo?.recycledAt).localeCompare(text(a.recycleInfo?.recycledAt))).map((batch) => { const summary = model.summarizeDisbursementBatch(batch); return `<tr><td><strong>${escapeHtml(batch.categoryName || templateLabel(batch.templateKey))}</strong><br><span class="text-secondary">${escapeHtml(batch.period)}</span></td><td>${money(summary.totalCents)} · ${summary.recipientCount} 人</td><td>${escapeHtml(batch.recycleInfo?.recycledAt || '')}</td><td><div class="cf-row-actions"><button data-cf-action="restore-disbursement-batch" data-id="${batch.id}">恢复</button><button class="btn btn-danger" data-cf-action="purge-disbursement-batch" data-id="${batch.id}">彻底清除</button></div></td></tr>`; }).join('');
    return `<div class="cf-panel"><div class="cf-panel-head"><div><h3>资金发放回收站</h3><span class="text-secondary">恢复后将回到原有状态；彻底清除不可恢复。</span></div></div><div class="cf-table-wrap"><table class="cf-table"><thead><tr><th>批次</th><th>金额 / 人数</th><th>移入时间</th><th>操作</th></tr></thead><tbody>${rows || '<tr><td colspan="4"><div class="cf-empty">回收站为空。</div></td></tr>'}</tbody></table></div></div>`;
  }
  ...
  function openModal(title, body, { small = false, footer = '' } = {}) {
    ...
    overlay.addEventListener('click', async (event) => {
      const previewPage = event.target.closest('[data-cf-preview-page]');
      if (previewPage && overlay.contains(previewPage)) {
        event.preventDefault();
        const page = Number(previewPage.dataset.cfPreviewPage || 1);
        const sheet = overlay.querySelector(`[data-cf-print-page="${page}"]`);
    ...
    overlay.addEventListener('change', async (event) => {
      const previewSetting = event.target.closest('[data-cf-preview-setting], [data-cf-preview-margin], [data-cf-preview-margin-preset]');
      if (previewSetting && overlay.contains(previewSetting)) {
        try {
          if (previewSetting.hasAttribute('data-cf-preview-margin-preset')) {
            const amount = ({ narrow: 8, standard: 12, wide: 18 })[previewSetting.value];
            if (amount) overlay.querySelectorAll('[data-cf-preview-margin]').forEach((input) => { input.value = amount; });
          }
          await updateTemplatePreviewSettings();
        } catch (error) { notify(error.message || '保存打印设置失败', 'error'); }
        return;
      }
      ...
      if (personControl && overlay.contains(personControl)) {
        const person = state.database.personnel.find((item) => model.personId(item) === personControl.value);
        if (person) fillTemplateResidentRow(personControl.closest('.cf-template-item'), person);
        return;
      }
      const nameControl = event.target.closest('[data-cf-template-name]');
      if (!nameControl || !overlay.contains(nameControl)) return;
      const row = nameControl.closest('.cf-template-item');
      const matches = state.database.personnel.filter((item) => model.personName(item) === text(nameControl.value));
      const status = row?.querySelector('[data-cf-template-resident-status]');
      if (matches.length === 1) {
        const person = matches[0]; const select = row?.querySelector('[data-cf-template-person]');
        if (select) select.value = model.personId(person);
        fillTemplateResidentRow(row, person);
      } else if (status) status.textContent = matches.length > 1 ? `发现 ${matches.length} 名同名居民，请从左侧下拉框确认组别和尾号` : '未找到居民档案，可继续手工填写';
    });
  ...
  function templateChooserModal() {
    ...
    openModal('选择发放模板', `<div class="cf-hint">先选择模板，再选择居民档案。银行卡、组别等资料会随所选居民自动带入；同名居民会显示组别和尾号，必须人工确认。</div><div class="cf-template-choices">${templates.map((template) => `<button data-cf-action="new-template-batch" data-template="${escapeHtml(template.key)}" data-template-id="${escapeHtml(template.id)}"><strong>${escapeHtml(template.name)}</strong><span>${escapeHtml(template.description || '可按实际业务填写发放明细')}</span><em>${escapeHtml(template.paper || 'A5')} · 每页 ${escapeHtml(template.rowsPerPage || 10)} 人</em></button>`).join('')}<button data-cf-action="import-disbursement-excel"><strong>导入现成 Excel 明细</strong><span>导入已有人员、金额和银行卡，再选择模板编辑与打印</span><em>Excel 导入</em></button><button data-cf-action="new-disbursement-generic"><strong>其他资金发放</strong><span>不套用打印模板的临时补贴或其他类别明细</span><em>通用</em></button></div>`, { footer: '<button class="btn btn-outline" data-cf-action="close-modal">取消</button>' });
  }

  async function importDisbursementExcel() {
    const result = await window.api.selectAndReadDisbursementExcel();
    if (!result?.ok) { if (!result?.canceled) notify(result?.error || 'Excel 导入失败', 'error'); return; }
    if (result.data?.requiresMapping) { state.pendingDisbursementMapping = result.data; return disbursementMappingModal(); }
    state.pendingDisbursementImport = result.data;
    notify(`已识别 ${result.data.total} 条发放明细，请选择模板后继续编辑`, 'success'); templateChooserModal();
  }

  function disbursementMappingModal() {
    const data = state.pendingDisbursementMapping; const options = (data?.columns || []).map((column, index) => `<option value="${index}">${escapeHtml(column || `第 ${index + 1} 列`)}</option>`).join('');
    openModal('确认 Excel 字段对应关系', `<div class="cf-hint">系统未能确认表头。请选择现成表格中的姓名、金额和银行卡列；其他列可暂不导入。</div><div class="cf-form-grid"><div class="cf-field"><label>姓名 *</label><select name="map-name">${options}</select></div><div class="cf-field"><label>金额 *</label><select name="map-amount">${options}</select></div><div class="cf-field"><label>银行卡</label><select name="map-bank"><option value="">本次不导入</option>${options}</select></div><div class="cf-field"><label>组别</label><select name="map-group"><option value="">本次不导入</option>${options}</select></div></div>`, { footer: '<button class="btn btn-outline" data-cf-action="close-modal">取消</button><button class="btn btn-primary" data-cf-action="confirm-disbursement-mapping">确认导入</button>' });
  }
  function confirmDisbursementMapping() { const data = state.pendingDisbursementMapping; const value = (name) => document.querySelector(`[name="${name}"]`)?.value; const nameIndex = Number(value('map-name')); const amountIndex = Number(value('map-amount')); const bankIndex = value('map-bank'); const groupIndex = value('map-group'); const rows = (data.rawGrid || []).filter((row) => text(row?.[nameIndex]) && !/^(合计|总计)/u.test(text(row?.[nameIndex]))).map((row, index) => ({ id: `mapped-import-${index + 1}`, name: text(row[nameIndex]), amount: text(row[amountIndex]), bankCard: bankIndex === '' ? '' : text(row[Number(bankIndex)]).replace(/[\s-]/gu, ''), groupName: groupIndex === '' ? '' : text(row[Number(groupIndex)]), rawData: {} })); if (!rows.length) throw new Error('未找到可导入人员，请重新确认列对应关系'); state.pendingDisbursementImport = { ...data, rows, total: rows.length, mapped: true }; state.pendingDisbursementMapping = null; notify(`已按确认字段导入 ${rows.length} 条明细，请选择模板`, 'success'); templateChooserModal(); }

  function templateColumnsText(template = {}) { return (template.columns || model.normalizeTemplateColumns(template)).map((column) => [column.label, column.source, column.residentField, column.calculation, column.width, column.visible === false ? '隐藏' : '显示'].join('|')).join('\n'); }
  function parseTemplateColumns(value, fields) { const labels = text(value).split(/\n/u).map((line) => line.trim()).filter(Boolean); if (!labels.length) return undefined; return labels.map((line, index) => { const [label, source = 'manual', residentField = '', calculation = '', width = '120', visible = '显示'] = line.split('|').map((part) => part.trim()); return { label: label || `字段${index + 1}`, source: ({ 居民: 'resident', 批次: 'batch', 手工: 'manual', 计算: 'calculation' })[source] || source, residentField, calculation, width, visible: visible !== '隐藏' }; }); }

  function templateManagementModal() {
    const rows = (state.database.disbursementTemplates || []).map((template) => `<tr><td><strong>${escapeHtml(template.name)}</strong><br><span class="text-secondary">${template.builtIn ? '默认模板（可修改）' : '自定义模板'}</span></td><td>${escapeHtml(template.paper)} · ${template.orientation === 'landscape' ? '横向' : '纵向'} · 每页 ${escapeHtml(template.rowsPerPage)} 人</td><td>${escapeHtml((template.fields || []).join('、'))}</td><td>${template.active === false ? statusBadge('unpaid') : statusBadge('completed')}</td><td><button data-cf-action="copy-template" data-id="${escapeHtml(template.id)}">复制</button><button data-cf-action="edit-template" data-id="${escapeHtml(template.id)}">修改</button>${template.builtIn ? `<button data-cf-action="reset-template" data-id="${escapeHtml(template.id)}">恢复原始</button>` : ''}</td></tr>`).join('');
    openModal('管理发放模板', `<div class="cf-hint">所有默认模板都可直接修改。修改只影响今后新建的批次，已保存或已发放的表格继续使用自己的版式快照；默认模板可随时恢复系统原始样式。</div><div class="cf-table-wrap"><table class="cf-table"><thead><tr><th>模板</th><th>打印</th><th>业务字段</th><th>状态</th><th>操作</th></tr></thead><tbody>${rows}</tbody></table></div>`, { footer: '<button class="btn btn-outline" data-cf-action="close-modal">关闭</button><button class="btn btn-primary" data-cf-action="new-template">＋ 新建自定义模板</button>' });
  }

  async function resetTemplate(id) {
    const current = findById('disbursementTemplates', id); const original = model.defaultDisbursementTemplates().find((template) => template.key === current?.key);
    if (!current || !original) throw new Error('该模板不支持恢复原始样式');
    if (!root.confirm(`确定恢复“${current.name}”的系统原始样式吗？这只影响今后新建的批次。`)) return;
    const next = model.normalizeDisbursementTemplate({ ...original, id: current.id, key: current.key, builtIn: true, createdAt: current.createdAt }, { id: current.id });
    state.database.disbursementTemplates.splice(state.database.disbursementTemplates.indexOf(current), 1, next); await saveDatabase('默认模板已恢复系统原始样式'); templateManagementModal();
  }

  function templateEditorModal(template = null) {
    const current = template || { name: '', categoryCode: '', paper: 'A5', orientation: 'portrait', rowsPerPage: 10, title: '', description: '', fields: ['事项', '数量', '单价', '金额', '备注'], showBankCard: true, showSigners: true };
    ...
    openModal(template ? `编辑模板 · ${template.name}` : '新建自定义发放模板', `<form id="cf-template-editor-form" class="cf-form-grid">${field('模板名称 *', `<input name="name" value="${escapeHtml(current.name)}" placeholder="例如：临时补贴发放表">`)}${field('资金类别', `<select name="categoryCode"><option value="">不限定类别</option>${categories.map((item) => `<option value="${escapeHtml(item.code)}"${selected(item.code, current.categoryCode)}>${escapeHtml(item.name)}</option>`).join('')}</select>`)}${field('表格打印标题 *', `<input name="title" value="${escapeHtml(current.title)}" placeholder="打印时显示的标题">`, true)}${field('打印纸张', `<select name="paper"><option value="A5"${selected(current.paper, 'A5')}>A5</option><option value="A4"${selected(current.paper, 'A4')}>A4</option></select>`)}${field('打印方向', `<select name="orientation"><option value="portrait"${selected(current.orientation, 'portrait')}>纵向</option><option value="landscape"${selected(current.orientation, 'landscape')}>横向</option></select>`)}${field('每页人数', `<input name="rowsPerPage" type="number" min="1" max="50" value="${escapeHtml(current.rowsPerPage)}">`)}${field('使用说明', `<input name="description" value="${escapeHtml(current.description)}" placeholder="说明该模板适用的发放事项">`, true)}${field('字段与列配置 *', `<textarea name="columnsConfig" placeholder="姓名|居民|name||120|显示\n金额|计算||amount|100|显示">${escapeHtml(templateColumnsText(current))}</textarea><p class="cf-hint">每行一个字段：显示名｜来源（居民／批次／手工／计算）｜居民字段｜计算方式｜列宽｜显示或隐藏。若字段来源为居民，选择居民后会自动带入对应资料。</p>`, true)}${field('显示选项', `<label class="cf-check"><input name="showBankCard" type="checkbox"${checked(current.showBankCard !== false)}>打印银行卡</label><label class="cf-check"><input name="showSigners" type="checkbox"${checked(current.showSigners !== false)}>显示签字栏</label>`, true)}<p id="cf-template-editor-error" class="cf-error" role="alert" style="display:none"></p></form>`, { footer: '<button class="btn btn-outline" data-cf-action="manage-templates">取消</button><button class="btn btn-primary" data-cf-action="save-template">保存模板</button>' });
  }
  ...
  async function saveTemplate() {
    ...
    try { const values = Object.fromEntries(new FormData(form).entries()); const current = findById('disbursementTemplates', state.modal?.templateId); const columns = parseTemplateColumns(values.columnsConfig); const normalized = { ...current, ...values, key: current?.key, fields: columns?.map((column) => column.label), columns, showBankCard: form.querySelector('[name="showBankCard"]')?.checked, showSigners: form.querySelector('[name="showSigners"]')?.checked }; const next = model.normalizeDisbursementTemplate(normalized, { id: current?.id }); if (current) state.database.disbursementTemplates.splice(state.database.disbursementTemplates.indexOf(current), 1, next); else state.database.disbursementTemplates.push(model.createDisbursementTemplate(normalized)); await saveDatabase('发放模板已保存'); templateManagementModal(); }
    catch (reason) { if (error) { error.textContent = reason.message || '保存模板失败'; error.style.display = 'block'; } else throw reason; }
  }

  function residentTemplateValue(person, field) { const key = text(field); if (['name', '姓名'].includes(key)) return model.personName(person); if (['group', 'groupName', '村民组', '组别'].includes(key)) return model.personGroup(person); if (['idCard', 'id_card', '身份证', '身份证号'].includes(key)) return text(person?.id_card || person?.idCard); if (['bankCard', 'cardNumber', '银行卡', '银行卡号'].includes(key)) return model.defaultBankCard(person); if (['phone', 'mobile', '手机号', '联系电话'].includes(key)) return text(person?.phone || person?.mobile || person?.contact_phone); return text(person?.[key]); }

  function batchIssueCards(batch) {
    const issues = model.disbursementBatchIssues(batch, state.database.personnel); const groups = [
      ['duplicate', '重复人员'], ['missing-bank-card', '缺少银行卡'], ['updated-bank-card', '银行卡有变化'], ['adjusted-amount', '手工调整金额'], ['ambiguous-person', '重名待确认'],
    ].map(([type, label]) => ({ label, count: issues.filter((item) => item.type === type).length })).filter((item) => item.count);
    return `<div class="cf-validation-cards">${groups.length ? groups.map((item) => `<button type="button" class="cf-validation-card" data-cf-action="preview-template-sync" data-id="${escapeHtml(batch.id)}"><strong>${item.count}</strong><span>${escapeHtml(item.label)}</span></button>`).join('') : '<span class="cf-badge ok">核对通过：未发现重复人员、缺卡或重名待确认</span>'}</div>`;
  }

  function fillTemplateResidentRow(row, person) {
    if (!row || !person) return;
    const fill = (name, value) => { const input = row.querySelector(`[name="${name}"]`); if (input) input.value = value || ''; };
    fill('name', model.personName(person)); fill('bankCard', model.defaultBankCard(person));
    row.querySelectorAll('[data-template-resident-field]').forEach((input) => { input.value = residentTemplateValue(person, input.dataset.templateResidentField); });
    const status = row.querySelector('[data-cf-template-resident-status]');
    if (status) status.textContent = model.defaultBankCard(person) ? '已从居民档案带入姓名、银行卡和关联字段' : '已关联居民档案；该居民暂未登记银行卡，可手工填写';
  }

  function templatePrintSettings(batch, template = {}) {
    const source = batch?.printSettings || {};
    const paper = text(source.paper || template.paper || (batch?.templateKey === 'contract_fee' ? 'A4' : 'A5')).toUpperCase();
    const clampMargin = (value) => { const parsed = Number(value); return Math.max(0, Math.min(40, Number.isFinite(parsed) ? parsed : 12)); };
    return {
      paper: ['A4', 'A5'].includes(paper) ? paper : 'A5', orientation: source.orientation === 'landscape' ? 'landscape' : (template.orientation === 'landscape' ? 'landscape' : 'portrait'),
      rowsPerPage: Math.max(1, Number(source.rowsPerPage || template.rowsPerPage || (paper === 'A4' ? 20 : 10))),
      margins: { top: clampMargin(source.margins?.top), bottom: clampMargin(source.margins?.bottom), left: clampMargin(source.margins?.left), right: clampMargin(source.margins?.right) },
    };
  }
  ...
  function templateItemHtml(templateKey, item = {}, template = null) {
    ...
    const selectedPerson = state.database.personnel.find((person) => model.personId(person) === item.personId);
    const fields = templateKey === 'position_salary'
      ...
      : templateKey === 'public_service'
        ...
        : isCustom
          ? (template?.columns?.filter((column) => column.visible !== false) || (template?.fields || []).map((label) => ({ label, source: 'manual' }))).map((column) => { const residentField = column.source === 'resident' ? (column.residentField || column.key) : ''; const value = item.customData?.[column.label] || (residentField && selectedPerson ? residentTemplateValue(selectedPerson, residentField) : ''); return `<input data-template-custom-field="${escapeHtml(column.label)}"${residentField ? ` data-template-resident-field="${escapeHtml(residentField)}"` : ''} value="${escapeHtml(value)}" placeholder="${escapeHtml(column.label)}">`; }).join('')
  ...
  async function templateBatchModal(templateKey, batch = null, templateId = '', editing = false) {
    ...
    openModal(batch ? (editableBatch ? `编辑 ${templateLabel(key)}` : `查看 ${templateLabel(key)}`) : `新建 ${templateLabel(key)}`, `<form id="cf-template-batch-form" class="cf-form-grid"><input type="hidden" name="categoryId" value="${escapeHtml(batch?.categoryId || category.id || '')}"><input type="hidden" name="categoryName" value="${escapeHtml(batch?.categoryName || category.name || '')}">
      ${field('表格标题 *', `<input name="title" value="${escapeHtml(title)}">`, true)}${field('发放期间 *', `<input name="period" value="${escapeHtml(batch?.period || `${new Date().getFullYear()} 年 ${new Date().getMonth() + 1} 月`)}">`)}${field('发放日期', `<input name="batchDate" type="date" value="${escapeHtml(batch?.batchDate || today())}">`)}${field('编制单位 / 村居', `<input name="villageName" value="${escapeHtml(batch?.villageName || state.database.settings?.villageName || '')}">`)}${field('每页人数', `<input name="rowsPerPage" type="number" min="1" max="50" value="${escapeHtml(batch?.printSettings?.rowsPerPage || template?.rowsPerPage || 10)}">`)}${field('审批人', `<input name="approver" value="${escapeHtml(batch?.signers?.approver || '')}">`)}${field('制表人', `<input name="maker" value="${escapeHtml(batch?.signers?.maker || '')}">`)}${field('经办人', `<input name="handler" value="${escapeHtml(batch?.signers?.handler || '')}">`)}${field('备注', `<input name="notes" value="${escapeHtml(batch?.notes || '')}">`, true)}${field('测试批次', `<label class="cf-check"><input name="isTest" type="checkbox"${checked(batch?.isTest)}>仅用于测试，后续可在回收站整批清理</label>`, true)}
      <div class="cf-field full">${batch ? batchIssueCards(batch) : '<div class="cf-hint">第 1 步选择模板与期间；第 2 步录入或导入人员；第 3 步核对资料并打印或登记发放。</div>'}<div class="cf-item-list-head"><label>发放明细</label>${!batch || editableBatch ? '<button type="button" class="btn btn-outline cf-add-template-item" data-cf-action="add-template-item">＋ 添加人员</button>' : ''}</div><div class="cf-hint">输入姓名后会自动关联唯一匹配的居民，并带入银行卡和已配置的号码字段；同名居民必须从带有组别和尾号的选项中手工选择。纸张、横竖方向和边距请在打印预览左侧调整。</div><div id="cf-template-items">${initialItems.map((item) => templateItemHtml(key, item, template)).join('')}</div><p id="cf-template-error" class="cf-error" role="alert" style="display:none"></p></div></form>`, { footer: editableBatch ? `<button class="btn btn-outline" data-cf-action="view-template-batch" data-id="${batch.id}">取消修改</button><button class="btn btn-primary" data-cf-action="save-template-batch">保存并预览</button>` : batch ? `<button class="btn btn-outline" data-cf-action="close-modal">关闭</button>${['draft', 'prepared'].includes(batch.status) ? `<button class="btn btn-outline" data-cf-action="edit-template-batch" data-id="${batch.id}">编辑表格</button>` : ''}<button class="btn btn-outline" data-cf-action="export-template-batch" data-id="${batch.id}">导出 Excel</button><button class="btn btn-primary" data-cf-action="preview-template" data-id="${batch.id}">打印预览</button>` : '<button class="btn btn-outline" data-cf-action="close-modal">取消</button><button class="btn btn-primary" data-cf-action="save-template-batch">保存并预览</button>' });
  }
  ...
  async function saveTemplateBatch() {
    ...
    try { const values = Object.fromEntries(new FormData(form).entries()); const template = templateFor(state.modal.templateId || state.modal.templateKey); const items = [...form.querySelectorAll('.cf-template-item')].map((row) => { const customData = Object.fromEntries([...row.querySelectorAll('[data-template-custom-field]')].map((input) => [input.dataset.templateCustomField, input.value])); return { personId: itemValue(row, 'personId'), name: itemValue(row, 'name'), idCard: row.dataset.importIdCard, groupName: row.dataset.importGroupName, phone: row.dataset.importPhone, bankName: row.dataset.importBankName, role: itemValue(row, 'role'), responsibilityArea: itemValue(row, 'responsibilityArea'), workDate: itemValue(row, 'workDate'), workItem: itemValue(row, 'workItem'), quantity: itemValue(row, 'quantity'), unitPrice: itemValue(row, 'unitPrice'), deductions: itemValue(row, 'deductions'), bankCard: itemValue(row, 'bankCard'), finalAmount: itemValue(row, 'finalAmount'), adjustmentReason: itemValue(row, 'adjustmentReason'), remark: itemValue(row, 'remark'), customData }; }).filter((item) => item.personId || item.name);
      const existing = findById('disbursementBatches', state.modal?.batchId); const settings = templatePrintSettings(existing, template); const created = model.createTemplateDisbursementBatch({ ...values, templateId: template?.id, templateSnapshot: template, templateKey: state.modal.templateKey, items, printSettings: { ...settings, rowsPerPage: values.rowsPerPage } }, { personnel: state.database.personnel, id: existing?.id }); const batch = existing ? { ...existing, ...created, id: existing.id, createdAt: existing.createdAt, status: 'draft', preparedAt: null, printedAt: null, reviewedAt: null, completedAt: null } : created;
      if (existing) state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(existing), 1, batch); else state.database.disbursementBatches.push(batch); state.pendingDisbursementImport = null; await saveDatabase(existing ? '发放表已修改，请重新核对打印预览' : '发放批次已保存，请核对打印预览'); templateBatchModal(batch.templateKey, batch, batch.templateId); }
  ...
  function subsidyRecordHtml(record = {}) {
    ...
    return `<tr class="cf-subsidy-record" data-id="${escapeHtml(record.id || '')}"><td><select name="personId"><option value="">手工匹配</option>${people}</select></td><td><input name="name" value="${escapeHtml(record.name || '')}"></td><td><input name="groupName" value="${escapeHtml(record.groupName || '')}"></td><td><select name="category"><option value="household"${selected(record.category, 'household')}>普通农户</option><option value="village_cadre"${selected(record.category, 'village_cadre')}>村干部</option></select></td><td><input name="eligibleArea" type="number" min="0" step="0.01" value="${escapeHtml(record.eligibleArea || '')}"></td><td><input name="standard" type="number" min="0" step="0.01" value="${record.standardCents === undefined ? '' : yuanValue(record.standardCents)}"></td><td><input name="idCard" value="${escapeHtml(record.idCard || '')}"></td><td><input name="phone" value="${escapeHtml(record.phone || '')}"></td><td><input name="bankName" value="${escapeHtml(record.bankName || '')}"></td><td><input name="bankCard" value="${escapeHtml(record.bankCard || '')}"></td></tr>`;
  }
  ...
  function subsidyUnresolvedRecords(ledger) { return typeof model.subsidyRecordsNeedingResidentSync === 'function' ? model.subsidyRecordsNeedingResidentSync(ledger) : (ledger?.records || []).filter((record) => record.matchStatus !== 'matched' || text(record.residentSyncStatus) !== 'synced'); }
  ...
  function subsidyResolutionModal(ledger, recordId, search = '') {
    ...
    const planned = model.subsidyResidentImportPlan(ledger, [record.id], state.database.personnel)[0]; const conflicts = planned?.conflicts || [];
    const currentPersonId = text(record.personId || planned?.personId); const currentPerson = state.database.personnel.find((person) => model.personId(person) === currentPersonId); const orderedCandidates = currentPerson && !candidates.some((candidate) => candidate.personId === currentPersonId) ? [{ person: currentPerson, personId: currentPersonId, reason: '当前关联居民' }, ...candidates] : candidates;
    state.modal = { type: 'subsidy-resolution', ledgerId: ledger.id, recordId, search };
    const candidateOptions = orderedCandidates.map((candidate) => `<option value="${escapeHtml(candidate.personId)}"${selected(candidate.personId, currentPersonId)}>${escapeHtml(model.personName(candidate.person))} · ${escapeHtml(model.personGroup(candidate.person) || '未分组')} · ${escapeHtml(candidate.reason)}</option>`).join('');
    const conflictRows = conflicts.length ? `<div class="cf-field full"><label>资料核对</label><table class="cf-table"><thead><tr><th>项目</th><th>居民档案</th><th>补贴表</th></tr></thead><tbody>${conflicts.map((item) => `<tr><td>${escapeHtml(item.field)}</td><td>${escapeHtml(item.residentValue || '未填写')}</td><td>${escapeHtml(item.subsidyValue || '未填写')}</td></tr>`).join('')}</tbody></table><small>请确认后选择保留原档案，或采用补贴表资料；系统不会自动覆盖。</small></div>` : '';
    openModal(`处理补贴关联 · ${record.name}`, `<div class="cf-hint">补贴数据保持不变。请只在确认是同一位居民时选择关联对象；同名不代表同一人。</div><div class="cf-form-grid"><div class="cf-field full"><label>补贴记录</label><div class="cf-record-summary"><strong>${escapeHtml(record.name)}</strong> · ${escapeHtml(record.groupName || '未分组')} · 身份证号 ${escapeHtml(record.idCard || '未填写')} · ${escapeHtml(record.eligibleArea)} 亩 · ${money(record.amountCents)}</div></div><div class="cf-field full"><label>搜索居民档案</label><div class="cf-subsidy-search"><input id="cf-subsidy-association-query" value="${escapeHtml(search)}" placeholder="输入居民姓名、身份证号或组别"><button type="button" class="btn btn-outline" data-cf-action="search-subsidy-association">搜索居民</button></div></div><div class="cf-field full"><label>确认关联居民 *</label><select id="cf-subsidy-person-choice"><option value="">请选择居民档案</option>${candidateOptions}</select><small>${search ? `搜索到 ${orderedCandidates.length} 名居民` : (orderedCandidates.length ? '已按身份证号、同组同名和同名顺序推荐' : '没有推荐候选，请搜索全体居民')}</small></div>${conflictRows}<div class="cf-field full"><label>资料处理方式</label><select id="cf-subsidy-conflict-resolution"><option value="keep">保留居民档案资料（仅补空白）</option><option value="adopt">采用补贴表资料</option></select></div><div class="cf-field full"><label>处理说明 *</label><input id="cf-subsidy-association-reason" value="${escapeHtml(record.associationNote || '')}" placeholder="例如：已核对身份证号后确认关联；或确认采用补贴表资料"></div><p id="cf-subsidy-association-error" class="cf-error" role="alert" style="display:none"></p></div>`, { footer: '<button class="btn btn-outline" data-cf-action="back-subsidy-issues">返回清单</button><button class="btn btn-outline" data-cf-action="defer-subsidy-association">暂不关联</button><button class="btn btn-primary" data-cf-action="confirm-subsidy-association">确认并同步</button>' });
  }
  ...
  async function saveSubsidyAssociation(deferred = false) {
    ...
    try {
      const ledger = findById('farmlandSubsidyLedgers', state.modal?.ledgerId); const record = ledger?.records?.find((item) => item.id === state.modal?.recordId); const reason = text(document.getElementById('cf-subsidy-association-reason')?.value); const personId = text(document.getElementById('cf-subsidy-person-choice')?.value); const resolution = text(document.getElementById('cf-subsidy-conflict-resolution')?.value) || 'keep';
      if (!reason) throw new Error('请填写处理说明'); if (!deferred && !personId) throw new Error('请选择确认关联的居民档案');
      if (deferred) {
        const next = model.correctFarmlandSubsidyRecord(ledger, record.id, { ...record, personId: '', associationStatus: 'deferred', associationNote: reason, correctionReason: reason }, { personnel: state.database.personnel });
        state.database.farmlandSubsidyLedgers.splice(state.database.farmlandSubsidyLedgers.indexOf(ledger), 1, next); await saveDatabase('已标记为暂不关联，仍会保留在待处理清单中'); return subsidyIssueListModal(next);
      }
      const result = model.resolveFarmlandSubsidyResidentConflict({ ledger, recordId: record.id, personId, personnel: state.database.personnel, resolution });
      const nextRecord = result.ledger.records.find((item) => item.id === record.id); nextRecord.associationNote = reason;
      state.database.personnel = result.personnel; state.database.farmlandSubsidyLedgers.splice(state.database.farmlandSubsidyLedgers.indexOf(ledger), 1, result.ledger); await saveDatabase(resolution === 'adopt' ? '已确认采用补贴表资料并同步居民档案' : '已确认关联并保留居民档案资料'); subsidyIssueListModal(result.ledger);
    } catch (reason) { if (error) { error.textContent = reason.message || '保存失败'; error.style.display = 'block'; } else throw reason; }
  }
  ...
  function subsidyRecordEditModal(ledger, recordId, query = '', page = 1, pageSize = 10) {
    ...
    openModal(`更正补贴记录 · ${record.name}`, `<form id="cf-subsidy-record-form" class="cf-form-grid">${field('关联居民', `<select name="personId"><option value="">保持手工资料</option>${people}</select>`)}${field('姓名 *', `<input name="name" value="${escapeHtml(record.name)}">`)}${field('村民组', `<input name="groupName" value="${escapeHtml(record.groupName)}">`)}${field('类别', `<select name="category"><option value="household"${selected(record.category, 'household')}>普通农户</option><option value="village_cadre"${selected(record.category, 'village_cadre')}>村干部</option></select>`)}${field('确权面积（亩）', `<input name="ownershipArea" type="number" min="0" step="0.01" value="${escapeHtml(record.ownershipArea)}">`)}${field('排除面积（亩）', `<input name="excludedArea" type="number" min="0" step="0.01" value="${escapeHtml(record.excludedArea)}">`)}${field('应补面积（亩）', `<input name="eligibleArea" type="number" min="0" step="0.01" value="${escapeHtml(record.eligibleArea)}">`)}${field('补贴标准（元/亩）', `<input name="standard" type="number" min="0" step="0.01" value="${escapeHtml(yuanValue(record.standardCents))}">`)}${field('身份证号', `<input name="idCard" value="${escapeHtml(record.idCard)}">`)}${field('手机号', `<input name="phone" value="${escapeHtml(record.phone || '')}">`)}${field('开户行', `<input name="bankName" value="${escapeHtml(record.bankName)}">`)}${field('一卡通号', `<input name="bankCard" value="${escapeHtml(record.bankCard)}">`)}${field('更正原因 *', '<input name="correctionReason" placeholder="例如：核实身份证号、面积、手机号或银行卡">', true)}<p id="cf-subsidy-record-error" class="cf-error" role="alert" style="display:none"></p></form>`, { footer: '<button class="btn btn-outline" data-cf-action="back-subsidy-editor">返回列表</button><button class="btn btn-primary" data-cf-action="save-subsidy-record">保存更正</button>' });
  }
  ...
  function subsidyRowsFromForm(form) {
    return [...form.querySelectorAll('.cf-subsidy-record')].map((row) => ({ id: row.dataset.id, personId: itemValue(row, 'personId'), name: itemValue(row, 'name'), groupName: itemValue(row, 'groupName'), category: itemValue(row, 'category'), eligibleArea: itemValue(row, 'eligibleArea'), ownershipArea: itemValue(row, 'eligibleArea'), standard: itemValue(row, 'standard'), idCard: itemValue(row, 'idCard'), phone: itemValue(row, 'phone'), bankName: itemValue(row, 'bankName'), bankCard: itemValue(row, 'bankCard') })).filter((row) => row.personId || row.name);
  }
  ...
  async function saveSubsidyLedger() {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const changed = ['personId', 'name', 'groupName', 'category', 'eligibleArea', 'standardCents', 'idCard', 'phone', 'bankName', 'bankCard'].some((key) => String(candidate[key] ?? '') !== String(old[key] ?? ''));
  ...
  function templatePreviewHtml(batch, { interactive = false } = {}) {
    const template = templateFor(batch.templateId || batch.templateKey, batch) || {}; const custom = !template.builtIn; const settings = templatePrintSettings(batch, template); const canEdit = interactive && ['draft', 'prepared'].includes(batch.status);
    const baseHeaders = custom ? ['序号', '姓名', '组别', ...(template.fields || []), '银行卡号', '实发金额', '备注'] : batch.templateKey === 'casual_labor' ? ['序号', '用工日期', '姓名', '用工事项', '工日', '单价', '金额', '银行账号', '备注'] : batch.templateKey === 'public_service' ? ['序号', '姓名', '负责区域', '账号', '金额', '备注'] : ['序号', '姓名', '职务', '元/月', '合计月份', '扣除款', '实发金额', '账号', '备注'];
    const layout = batch.previewLayout || template.previewLayout || {}; const configuredOrder = Array.isArray(layout.order) ? layout.order : []; const hidden = new Set(Array.isArray(layout.hidden) ? layout.hidden : []);
    const orderedBaseHeaders = [...configuredOrder.filter((header) => baseHeaders.includes(header)), ...baseHeaders.filter((header) => !configuredOrder.includes(header))].filter((header) => !hidden.has(header));
    const headers = canEdit ? [...orderedBaseHeaders, '操作'] : orderedBaseHeaders;
    ...
    const total = model.summarizeDisbursementBatch(batch).totalCents; const paper = settings.paper.toLowerCase(); const perPage = settings.rowsPerPage; const pages = [];
    for (let index = 0; index < batch.items.length; index += perPage) pages.push(batch.items.slice(index, index + perPage));
    if (!pages.length) pages.push([]);
    const padding = `${settings.margins.top}mm ${settings.margins.right}mm ${settings.margins.bottom}mm ${settings.margins.left}mm`;
    return pages.map((items, pageIndex) => { const rows = items.map((item, index) => { const values = rowCells(item, pageIndex * perPage + index); const valueByHeader = new Map(baseHeaders.map((header, cellIndex) => [header, values[cellIndex]])); return `<tr>${orderedBaseHeaders.map((header) => `<td${layout.widths?.[header] ? ` style="width:${Math.max(60, Math.min(360, Number(layout.widths[header]) || 120))}px"` : ''}>${escapeHtml(valueByHeader.get(header) || '')}</td>`).join('')}${canEdit ? `<td class="cf-print-actions"><button data-cf-action="edit-template-preview-item" data-id="${escapeHtml(batch.id)}" data-item-id="${escapeHtml(item.id)}">修改</button><button data-cf-action="delete-template-preview-item" data-id="${escapeHtml(batch.id)}" data-item-id="${escapeHtml(item.id)}">删除</button></td>` : ''}</tr>`; }).join(''); const isLast = pageIndex === pages.length - 1; return `<article class="cf-print-sheet ${paper} ${settings.orientation}" style="padding:${padding}" data-cf-print-page="${pageIndex + 1}"><h1>${escapeHtml(batch.title || template.title || templateLabel(batch.templateKey))}</h1><div class="cf-print-meta"><span>编制单位：${escapeHtml(batch.villageName || '')}</span><span>期间：${escapeHtml(batch.period || '')}</span><span>日期：${escapeHtml(batch.batchDate || '')}</span><span>单位：元</span></div><table><thead><tr>${headers.map((header) => `<th${layout.widths?.[header] ? ` style="width:${Math.max(60, Math.min(360, Number(layout.widths[header]) || 120))}px"` : ''}>${escapeHtml(header)}</th>`).join('')}</tr></thead><tbody>${rows}${isLast ? `<tr><td colspan="${Math.max(1, headers.length - 3)}">合计</td><td colspan="3">${money(total)}　共 ${batch.items.length} 人</td></tr>` : ''}</tbody></table><div class="cf-print-signers"><span>审批人：${escapeHtml(batch.signers?.approver || '')}</span><span>制表人：${escapeHtml(batch.signers?.maker || '')}</span>${batch.templateKey === 'casual_labor' ? `<span>经办人：${escapeHtml(batch.signers?.handler || '')}</span>` : ''}<span>第 ${pageIndex + 1}/${pages.length} 页</span></div></article>`; }).join('');
  }

  function previewBaseHeaders(batch) {
    const template = templateFor(batch.templateId || batch.templateKey, batch) || {}; const custom = !template.builtIn;
    return custom ? ['序号', '姓名', '组别', ...(template.fields || []), '银行卡号', '实发金额', '备注'] : batch.templateKey === 'casual_labor' ? ['序号', '用工日期', '姓名', '用工事项', '工日', '单价', '金额', '银行账号', '备注'] : batch.templateKey === 'public_service' ? ['序号', '姓名', '负责区域', '账号', '金额', '备注'] : ['序号', '姓名', '职务', '元/月', '合计月份', '扣除款', '实发金额', '账号', '备注'];
  }

  function previewLayoutEditor(batch) {
    if (!['draft', 'prepared'].includes(batch?.status)) throw new Error('该批次已锁定，不能修改版式');
    const layout = batch.previewLayout || {}; const all = previewBaseHeaders(batch); const order = [...(layout.order || []).filter((header) => all.includes(header)), ...all.filter((header) => !(layout.order || []).includes(header))];
    state.modal = { type: 'preview-layout', batchId: batch.id };
    openModal('编辑打印列布局', `<div class="cf-hint">按住每一行左侧的拖动符号可调整列顺序；可修改列宽或暂时隐藏列。不会移动单元格，也不会合并单元格。</div><div id="cf-preview-layout-list">${order.map((header) => `<div class="cf-layout-field" draggable="true" data-cf-layout-field="${escapeHtml(header)}"><span class="cf-layout-drag" title="拖动调整顺序">⋮⋮</span><strong>${escapeHtml(header)}</strong><label>列宽 <input type="number" min="60" max="360" value="${escapeHtml(layout.widths?.[header] || 120)}"></label><label class="cf-check"><input type="checkbox"${checked(!(layout.hidden || []).includes(header))}>显示</label></div>`).join('')}</div>`, { footer: '<button class="btn btn-outline" data-cf-action="return-template-preview">取消</button><button class="btn btn-outline" data-cf-action="reset-preview-layout">恢复当前模板版式</button><button class="btn btn-outline" data-cf-action="save-preview-layout-template">保存到当前模板</button><button class="btn btn-primary" data-cf-action="save-preview-layout-copy">另存为新模板</button>' });
    const list = document.getElementById('cf-preview-layout-list'); let dragging = null;
    list?.querySelectorAll('[data-cf-layout-field]').forEach((row) => { row.addEventListener('dragstart', () => { dragging = row; row.classList.add('dragging'); }); row.addEventListener('dragend', () => { dragging?.classList.remove('dragging'); dragging = null; }); row.addEventListener('dragover', (event) => { event.preventDefault(); }); row.addEventListener('drop', (event) => { event.preventDefault(); if (dragging && dragging !== row) list.insertBefore(dragging, row); }); });
  }

  async function savePreviewLayout(reset = false) {
    const batch = findById('disbursementBatches', state.modal?.batchId); if (!batch) throw new Error('未找到发放批次');
    const layout = reset ? {} : { order: [...document.querySelectorAll('[data-cf-layout-field]')].map((row) => row.dataset.cfLayoutField), hidden: [...document.querySelectorAll('[data-cf-layout-field]')].filter((row) => !row.querySelector('[type="checkbox"]')?.checked).map((row) => row.dataset.cfLayoutField), widths: Object.fromEntries([...document.querySelectorAll('[data-cf-layout-field]')].map((row) => [row.dataset.cfLayoutField, Math.max(60, Math.min(360, Number(row.querySelector('[type="number"]')?.value) || 120))])) };
    const next = { ...structuredClone(batch), previewLayout: layout, updatedAt: new Date().toISOString() }; state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(batch), 1, next); await saveDatabase(reset ? '已恢复当前模板的默认列布局' : '列布局已保存到当前批次'); templatePreviewModal(next);
  }

  function readPreviewLayoutForm() { return { order: [...document.querySelectorAll('[data-cf-layout-field]')].map((row) => row.dataset.cfLayoutField), hidden: [...document.querySelectorAll('[data-cf-layout-field]')].filter((row) => !row.querySelector('[type="checkbox"]')?.checked).map((row) => row.dataset.cfLayoutField), widths: Object.fromEntries([...document.querySelectorAll('[data-cf-layout-field]')].map((row) => [row.dataset.cfLayoutField, Math.max(60, Math.min(360, Number(row.querySelector('[type="number"]')?.value) || 120))])) }; }
  async function savePreviewLayoutTemplate(copy = false) {
    const batch = findById('disbursementBatches', state.modal?.batchId); const current = templateFor(batch?.templateId || batch?.templateKey, batch); if (!batch || !current) throw new Error('未找到对应模板'); const layout = readPreviewLayoutForm();
    if (copy) { const next = model.createDisbursementTemplate({ ...structuredClone(current), id: '', key: '', builtIn: false, name: `${current.name}（版式副本）`, title: `${current.title || current.name}（版式副本）`, previewLayout: layout, createdAt: null, updatedAt: null }); state.database.disbursementTemplates.push(next); await saveDatabase('已另存为新的发放模板'); }
    else { const next = model.normalizeDisbursementTemplate({ ...current, previewLayout: layout }, { id: current.id }); state.database.disbursementTemplates.splice(state.database.disbursementTemplates.indexOf(current), 1, next); await saveDatabase('已保存到当前默认模板'); }
  ...
  async function printTemplate() { const current = findById('disbursementBatches', state.modal?.batchId); if (!current) return; const template = templateFor(current.templateId || current.templateKey, current) || {}; const settings = templatePrintSettings(current, template); const source = templatePreviewHtml(current); const pageSize = `${settings.paper} ${settings.orientation}`; const popup = root.open('', '_blank'); if (!popup) throw new Error('未能打开打印窗口，请允许本软件打开打印预览'); popup.document.write(`<html><head><title>发放表打印</title><style>body{margin:0;font-family:"Songti SC",serif;color:#000}.cf-print-sheet{box-sizing:border-box;margin:0 auto;page-break-after:always}.cf-print-sheet:last-child{page-break-after:auto}.cf-print-sheet.a5{width:148mm;min-height:210mm}.cf-print-sheet.a4{width:210mm;min-height:297mm}.cf-print-sheet.a5.landscape{width:210mm;min-height:148mm}.cf-print-sheet.a4.landscape{width:297mm;min-height:210mm}.cf-print-sheet h1{text-align:center;font-size:18pt;margin:0 0 8mm}.cf-print-meta,.cf-print-signers{display:flex;justify-content:space-between;gap:8px;margin:4mm 0;font-size:10pt}.cf-print-sheet table{border-collapse:collapse;width:100%;font-size:9pt}.cf-print-sheet th,.cf-print-sheet td{border:1px solid #000;padding:2.2mm 1.5mm;text-align:center;word-break:break-all}@page{size:${pageSize};margin:0}@media print{.cf-print-sheet{margin:0}}</style></head><body>${source}<script>window.onload=()=>window.print()<\/script></body></html>`); popup.document.close(); if (typeof model.markTemplateDisbursementPrinted === 'function') { const next = model.markTemplateDisbursementPrinted(current); state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1, next); await saveDatabase('已登记打印时间'); } }
  async function updateTemplatePreviewSettings() {
    const batch = findById('disbursementBatches', state.modal?.batchId); if (!batch || !['draft', 'prepared'].includes(batch.status)) throw new Error('该批次已锁定，不能修改打印设置');
    const overlay = document.getElementById('cf-modal-overlay'); const template = templateFor(batch.templateId || batch.templateKey, batch) || {}; const current = templatePrintSettings(batch, template);
    const read = (selector, fallback) => overlay?.querySelector(selector)?.value || fallback;
    const number = (selector, fallback) => { const parsed = Number(read(selector, fallback)); return Math.max(0, Math.min(40, Number.isFinite(parsed) ? parsed : fallback)); };
    const printSettings = {
      paper: read('[data-cf-preview-setting="paper"]', current.paper), orientation: read('[data-cf-preview-setting="orientation"]', current.orientation),
      rowsPerPage: Math.max(1, Math.min(50, Number(read('[data-cf-preview-setting="rowsPerPage"]', current.rowsPerPage)) || current.rowsPerPage)),
      margins: { top: number('[data-cf-preview-margin="top"]', current.margins.top), bottom: number('[data-cf-preview-margin="bottom"]', current.margins.bottom), left: number('[data-cf-preview-margin="left"]', current.margins.left), right: number('[data-cf-preview-margin="right"]', current.margins.right) },
    };
    const next = { ...structuredClone(batch), printSettings, updatedAt: new Date().toISOString() };
    state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(batch), 1, next); await saveDatabase(); templatePreviewModal(next);
  }

  function templateItemValues(row) {
    const customData = Object.fromEntries([...row.querySelectorAll('[data-template-custom-field]')].map((input) => [input.dataset.templateCustomField, input.value]));
    return { personId: itemValue(row, 'personId'), name: itemValue(row, 'name'), idCard: row.dataset.importIdCard, groupName: row.dataset.importGroupName, phone: row.dataset.importPhone, bankName: row.dataset.importBankName, role: itemValue(row, 'role'), responsibilityArea: itemValue(row, 'responsibilityArea'), workDate: itemValue(row, 'workDate'), workItem: itemValue(row, 'workItem'), quantity: itemValue(row, 'quantity'), unitPrice: itemValue(row, 'unitPrice'), deductions: itemValue(row, 'deductions'), bankCard: itemValue(row, 'bankCard'), finalAmount: itemValue(row, 'finalAmount'), adjustmentReason: itemValue(row, 'adjustmentReason'), remark: itemValue(row, 'remark'), customData };
  }

  function editTemplatePreviewItem(batchId, itemId) {
    const batch = findById('disbursementBatches', batchId); const item = batch?.items?.find((entry) => entry.id === itemId); if (!batch || !item || !['draft', 'prepared'].includes(batch.status)) throw new Error('该明细已锁定，不能修改');
    const template = templateFor(batch.templateId || batch.templateKey, batch); state.modal = { type: 'template-preview-item', batchId, itemId };
    openModal(`修改明细 · ${item.name}`, `<form id="cf-preview-item-form"><div id="cf-template-items">${templateItemHtml(batch.templateKey, item, template)}</div><p class="cf-hint">输入姓名后会自动关联唯一匹配的居民；同名人员请在第一个下拉框中确认。</p><p id="cf-template-error" class="cf-error" role="alert" style="display:none"></p></form>`, { footer: '<button class="btn btn-outline" data-cf-action="return-template-preview">取消</button><button class="btn btn-primary" data-cf-action="save-template-preview-item">保存修改</button>' });
  }

  async function saveTemplatePreviewItem() {
    const batch = findById('disbursementBatches', state.modal?.batchId); const oldItem = batch?.items?.find((item) => item.id === state.modal?.itemId); const row = document.querySelector('#cf-preview-item-form .cf-template-item'); if (!batch || !oldItem || !row) throw new Error('待修改的明细不存在');
    const nextItem = model.templateItem({ ...oldItem, ...templateItemValues(row) }, state.database.personnel, batch.templateKey, { id: oldItem.id });
    const next = { ...structuredClone(batch), items: batch.items.map((item) => item.id === oldItem.id ? { ...nextItem, createdAt: oldItem.createdAt } : item), status: 'draft', preparedAt: null, printedAt: null, updatedAt: new Date().toISOString() };
    state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(batch), 1, next); await saveDatabase('明细已修改，已重新生成打印预览'); templatePreviewModal(next);
  }

  async function deleteTemplatePreviewItem(batchId, itemId) {
    const batch = findById('disbursementBatches', batchId); if (!batch || !['draft', 'prepared'].includes(batch.status)) throw new Error('该批次已锁定，不能删除'); if (batch.items.length <= 1) throw new Error('发放表至少需要保留一名人员，不能删除最后一条明细');
    const item = batch.items.find((entry) => entry.id === itemId); if (!item || !root.confirm(`确定删除“${item.name}”这条发放明细吗？`)) return;
  ...
  function templateResidentSyncModal(batch, completeAfter = false) {
    ...
    const rows = plan.map((entry) => { const person = entry.person; const choices = (entry.candidates || []).map((candidate) => `<option value="${escapeHtml(candidate.personId)}"${selected(candidate.personId, entry.personId)}>${escapeHtml(model.personName(candidate.person))} · ${escapeHtml(model.personGroup(candidate.person) || '未分组')}</option>`).join(''); const differentCard = person && entry.item.bankCard && model.defaultBankCard(person) && model.defaultBankCard(person) !== model.normalizeBankCard(entry.item.bankCard); const personChoice = person ? `已关联：${escapeHtml(model.personName(person) || '')}` : `<select name="personId"><option value="">请选择居民</option>${choices}</select>`; const cardChoice = differentCard ? `<select name="bankCardDecision"><option value="">请选择银行卡处理方式</option><option value="once"${selected(entry.bankCardDecision, 'once')}>仅本次使用，不写入档案</option><option value="add"${selected(entry.bankCardDecision, 'add')}>新增为备用卡（推荐）</option><option value="default"${selected(entry.bankCardDecision, 'default')}>设为默认收款卡</option></select>` : (entry.item.bankCard ? '无冲突，发放完成后写入档案' : '本次无银行卡'); return `<tr data-template-resident-sync="${escapeHtml(entry.itemId)}"><td><strong>${escapeHtml(entry.item.name)}</strong><br><span class="text-secondary">${escapeHtml(entry.item.groupName || '未分组')}</span></td><td>${escapeHtml(entry.item.idCard || '未填写')}<br>${escapeHtml(entry.item.phone || '—')}</td><td>${escapeHtml(entry.item.bankCard || '—')}<br>${escapeHtml(entry.item.bankName || '—')}</td><td>${escapeHtml(entry.reason || (differentCard ? '银行卡与默认卡不同' : '资料已核对'))}</td><td>${personChoice}<br>${cardChoice}</td></tr>`; }).join('');
    openModal('发放完成前的居民资料核对', `<div class="cf-hint">此处只核对和保存确认选择，尚不会改动居民档案。只有本批次点击“发放完成”后，银行卡、资料来源和个人资金记录才会正式写入。</div><div class="cf-table-wrap"><table class="cf-table"><thead><tr><th>本次收款人</th><th>证件 / 手机号</th><th>银行卡 / 开户行</th><th>处理说明</th><th>确认方式</th></tr></thead><tbody>${rows}</tbody></table></div>`, { footer: `<button class="btn btn-outline" data-cf-action="close-modal">取消</button><button class="btn btn-primary" data-cf-action="confirm-template-resident-sync">${completeAfter ? '确认并发放完成' : '保存核对结果'}</button>` });
  }
  async function completeTemplateBatch(id) { const current = findById('disbursementBatches', id); if (!current) throw new Error('未找到发放批次'); const resolutions = current.residentSyncDecisions || {}; const plan = model.disbursementResidentSyncPlan(current, state.database.personnel, resolutions); if (plan.some((entry) => ['manual', 'missing'].includes(entry.status))) return templateResidentSyncModal(current, true); const result = model.completeTemplateDisbursementBatch(current, { personnel: state.database.personnel, resolutions }); state.database.personnel = result.personnel; state.database.residentOperationLog.unshift(...result.operationEntries); state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1, result.batch); await saveDatabase('已完成发放并写入居民资金与工作记录'); closeModal(); renderShell(); }
  async function confirmTemplateResidentSync() {
    const current = findById('disbursementBatches', state.modal?.batchId); const completeAfter = Boolean(state.modal?.completeAfter); const resolutions = {};
    document.querySelectorAll('[data-template-resident-sync]').forEach((row) => { const personId = row.querySelector('[name="personId"]')?.value; const bankCardDecision = row.querySelector('[name="bankCardDecision"]')?.value; if (personId || bankCardDecision) resolutions[row.dataset.templateResidentSync] = { personId, bankCardDecision }; });
    const next = { ...current, residentSyncDecisions: { ...(current.residentSyncDecisions || {}), ...resolutions }, updatedAt: new Date().toISOString() }; state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1, next); await saveDatabase('已保存居民资料核对结果，发放完成后才会正式同步');
    if (completeAfter) return completeTemplateBatch(next.id);
    closeModal(); renderShell();
  }
  ...
  function disbursementBatchModal(batch = null) {
    ...
    openModal(batch ? '查看发放批次' : '新建发放批次', `<form id="cf-disbursement-form" class="cf-form-grid" data-id="${escapeHtml(batch?.id || '')}">
      ...
      ${batch ? '' : field('测试批次', '<label class="cf-check"><input type="checkbox" name="isTest">仅用于测试，后续可在回收站整批清理</label>', true)}
  ...
  async function importDisbursementExcel() {
    const result = await api.selectAndReadDisbursementExcel(); if (!result?.ok) return;
    ...
    rows.forEach((row) => appendDisbursementItem({ name: row.name, groupName: row.groupName, idCard: row.idCard, phone: row.phone, bankName: row.bankName, bankCard: row.bankCard, amount: row.amount }));
  ...
  async function saveDisbursementBatch() {
    ...
    try {
      ...
      const items = [...form.querySelectorAll('.cf-disbursement-item')].map((row) => ({ personId: row.querySelector('[name="personId"]').value, name: row.querySelector('[name="name"]').value, groupName: row.dataset.importGroupName, idCard: row.dataset.importIdCard, phone: row.dataset.importPhone, bankName: row.dataset.importBankName, bankCard: row.querySelector('[name="bankCard"]').value, amount: row.querySelector('[name="amount"]').value })).filter((item) => item.personId || item.name || item.amount);
  ...
  async function payDisbursement(id) { return completeTemplateBatch(id); }
  async function copyDisbursementBatch(id) { const current = findById('disbursementBatches', id); const next = model.copyDisbursementBatch(current, state.database.personnel); state.database.disbursementBatches.push(next); await saveDatabase('已复制为新的草稿批次，并按居民档案重新带入银行卡'); state.view = 'general-batches'; renderShell(); }
  async function recycleDisbursementBatch(id) { const current = findById('disbursementBatches', id); if (!current) throw new Error('未找到发放批次'); const summary = model.summarizeDisbursementBatch(current); if (!root.confirm(`确定将“${current.categoryName || templateLabel(current.templateKey)}（${current.period}）”移入回收站吗？\n共 ${summary.recipientCount} 人，${money(summary.totalCents)}。`)) return; const recycled = model.recycleDisbursementBatch(current); state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1); state.database.disbursementRecycleBin.push(recycled); await saveDatabase('发放批次已移入回收站，可随时恢复'); renderShell(); }
  async function restoreDisbursementBatch(id) { const current = findById('disbursementRecycleBin', id); if (!current) throw new Error('未找到回收站批次'); const restored = model.restoreDisbursementBatch(current); state.database.disbursementRecycleBin.splice(state.database.disbursementRecycleBin.indexOf(current), 1); state.database.disbursementBatches.push(restored); await saveDatabase('发放批次已恢复'); renderShell(); }
  async function purgeDisbursementBatch(id) { const current = findById('disbursementRecycleBin', id); if (!current) throw new Error('未找到回收站批次'); const summary = model.summarizeDisbursementBatch(current); if (!root.confirm(`彻底清除“${current.categoryName || templateLabel(current.templateKey)}（${current.period}）”吗？\n共 ${summary.recipientCount} 人，${money(summary.totalCents)}。此操作不能恢复。`)) return; state.database.disbursementRecycleBin.splice(state.database.disbursementRecycleBin.indexOf(current), 1); await saveDatabase('已彻底清除该发放批次'); renderShell(); }
  ...
  async function handleAction(action, element) {
    ...
    if (action === 'copy-template') { const source = findById('disbursementTemplates', element.dataset.id); return templateEditorModal({ ...structuredClone(source), id: '', key: '', builtIn: false, name: `${source.name}（副本）`, title: `${source.title || source.name}（副本）`, createdAt: null, updatedAt: null }); }
    if (action === 'edit-template') return templateEditorModal(findById('disbursementTemplates', element.dataset.id));
    if (action === 'reset-template') return resetTemplate(element.dataset.id);
    ...
    if (action === 'confirm-disbursement-mapping') return confirmDisbursementMapping();
    ...
    if (action === 'view-template-batch') { const current = findById('disbursementBatches', element.dataset.id); return templateBatchModal(current.templateKey, current, current.templateId); }
    if (action === 'edit-template-batch') { const current = findById('disbursementBatches', element.dataset.id); return templateBatchModal(current.templateKey, current, current.templateId, true); }
    if (action === 'export-template-batch') return exportTemplateBatch(element.dataset.id);
    if (action === 'preview-template') return templatePreviewModal(findById('disbursementBatches', element.dataset.id));
    if (action === 'edit-preview-layout') return previewLayoutEditor(findById('disbursementBatches', element.dataset.id));
    if (action === 'save-preview-layout') return savePreviewLayout(false);
    if (action === 'reset-preview-layout') return savePreviewLayout(true);
    if (action === 'save-preview-layout-template') return savePreviewLayoutTemplate(false);
    if (action === 'save-preview-layout-copy') return savePreviewLayoutTemplate(true);
    if (action === 'edit-template-preview-item') return editTemplatePreviewItem(element.dataset.id, element.dataset.itemId);
    if (action === 'delete-template-preview-item') return deleteTemplatePreviewItem(element.dataset.id, element.dataset.itemId);
    if (action === 'save-template-preview-item') return saveTemplatePreviewItem();
    if (action === 'return-template-preview') return templatePreviewModal(findById('disbursementBatches', state.modal?.batchId));
    ...
    if (action === 'complete-template') return completeTemplateBatch(element.dataset.id);
    if (action === 'preview-template-sync' || action === 'sync-template-batch') return templateResidentSyncModal(findById('disbursementBatches', element.dataset.id));
    if (action === 'confirm-template-resident-sync') return confirmTemplateResidentSync();
    if (action === 'copy-disbursement-batch') return copyDisbursementBatch(element.dataset.id);
    if (action === 'recycle-disbursement-batch') return recycleDisbursementBatch(element.dataset.id);
    if (action === 'restore-disbursement-batch') return restoreDisbursementBatch(element.dataset.id);
    if (action === 'purge-disbursement-batch') return purgeDisbursementBatch(element.dataset.id);
    if (action === 'query-disbursement-batches') { state.disbursementQuery = document.getElementById('cf-disbursement-query')?.value || ''; return renderView(); }
    if (action === 'clear-disbursement-query') { state.disbursementQuery = ''; return renderView(); }
