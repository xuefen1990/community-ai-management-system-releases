...
(function contractFeeWorkspace(root) {
  ...
  function subsidyDetailsModal(ledger, sheet = 'payment', page = 1, query = '') {
    if (!ledger) throw new Error('未找到年度地力补贴台账');
    const data = subsidySheetData(ledger, sheet); const matchedRows = subsidyQueryRows(data.rows, query); const pageSize = 50; const totalPages = Math.max(1, Math.ceil(matchedRows.length / pageSize)); const currentPage = Math.min(Math.max(Number(page) || 1, 1), totalPages); const first = (currentPage - 1) * pageSize; const rows = matchedRows.slice(first, first + pageSize);
    const tabs = [['payment', '主表'], ['attachment-1-1', '附件 1-1'], ['attachment-1-4', '附件 1-4'], ['attachment-2-1', '附件 2-1'], ['attachment-2-4', '附件 2-4']];
    state.modal = { type: 'subsidy-detail', ledgerId: ledger.id, sheet, page: currentPage, query: text(query) };
    openModal(`${ledger.year} 年地力补贴 · 主表与附件`, `<div class="cf-hint">${escapeHtml(data.label)}。查看不会改动已导入的数据；导出 Excel 时使用同一套关联数据。</div><div class="cf-subsidy-search"><input id="cf-subsidy-query" value="${escapeHtml(query)}" placeholder="输入姓名、身份证号、银行卡号或组别"><button class="btn btn-primary" data-cf-action="search-subsidy-sheet">查询定位</button>${query ? '<button class="btn btn-outline" data-cf-action="clear-subsidy-sheet-search">清除</button>' : ''}</div><div class="cf-sheet-tabs">${tabs.map(([key, label]) => `<button class="${key === sheet ? 'active' : ''}" data-cf-action="view-subsidy-sheet" data-sheet="${key}">${label}</button>`).join('')}</div><div class="cf-table-wrap"><table class="cf-table"><thead><tr>${data.headers.map((header) => `<th>${escapeHtml(header)}</th>`).join('')}</tr></thead><tbody>${rows.length ? rows.map((row) => `<tr>${row.map((cell) => `<td>${escapeHtml(cell ?? '')}</td>`).join('')}</tr>`).join('') : `<tr><td colspan="${data.headers.length}"><div class="cf-empty">没有符合查询条件的数据。</div></td></tr>`}</tbody></table></div><div class="cf-pager"><span>${query ? `查询到 ${matchedRows.length} 条` : `共 ${data.rows.length} 条`}，第 ${currentPage}/${totalPages} 页</span><div><button data-cf-action="view-subsidy-page" data-page="${currentPage - 1}"${currentPage <= 1 ? ' disabled' : ''}>上一页</button><button data-cf-action="view-subsidy-page" data-page="${currentPage + 1}"${currentPage >= totalPages ? ' disabled' : ''}>下一页</button></div></div>`, { footer: `<button class="btn btn-outline" data-cf-action="close-modal">关闭</button><button class="btn btn-primary" data-cf-action="edit-subsidy" data-id="${ledger.id}">编辑主表</button>` });
  }

  function subsidyRecordListModal(ledger, page = 1, query = '') {
    if (!ledger) throw new Error('未找到年度地力补贴台账');
    const needle = text(query).toLowerCase(); const all = ledger.records || []; const matched = needle ? all.filter((record) => [record.name, record.idCard, record.bankCard, record.groupName].some((value) => text(value).toLowerCase().includes(needle))) : all; const pageSize = 30; const totalPages = Math.max(1, Math.ceil(matched.length / pageSize)); const currentPage = Math.min(Math.max(Number(page) || 1, 1), totalPages); const rows = matched.slice((currentPage - 1) * pageSize, currentPage * pageSize);
    state.modal = { type: 'subsidy-editor-list', ledgerId: ledger.id, page: currentPage, query: text(query) };
    openModal(`${ledger.year} 年地力补贴 · 编辑主表`, `<div class="cf-hint">请先查询定位人员，再点“编辑”更正单条数据。不会一次性加载全部编辑控件，数据多也保持流畅；保存更正时必须填写原因。</div><div class="cf-subsidy-search"><input id="cf-subsidy-editor-query" value="${escapeHtml(query)}" placeholder="输入姓名、身份证号、银行卡号或组别"><button class="btn btn-primary" data-cf-action="search-subsidy-editor">查询定位</button>${query ? '<button class="btn btn-outline" data-cf-action="clear-subsidy-editor-search">清除</button>' : ''}</div><div class="cf-table-wrap"><table class="cf-table"><thead><tr><th>序号</th><th>姓名</th><th>身份证号</th><th>村民组</th><th>一卡通号</th><th>应补面积</th><th>补贴金额</th><th>操作</th></tr></thead><tbody>${rows.length ? rows.map((record, index) => `<tr><td>${(currentPage - 1) * pageSize + index + 1}</td><td><strong>${escapeHtml(record.name)}</strong></td><td>${escapeHtml(record.idCard || '—')}</td><td>${escapeHtml(record.groupName || '—')}</td><td>${escapeHtml(record.bankCard || '—')}</td><td>${escapeHtml(record.eligibleArea)}</td><td>${money(record.amountCents)}</td><td><button data-cf-action="edit-subsidy-record" data-id="${escapeHtml(record.id)}">编辑</button></td></tr>`).join('') : '<tr><td colspan="8"><div class="cf-empty">没有符合查询条件的数据。</div></td></tr>'}</tbody></table></div><div class="cf-pager"><span>${query ? `查询到 ${matched.length} 条` : `共 ${all.length} 条`}，第 ${currentPage}/${totalPages} 页</span><div><button data-cf-action="subsidy-editor-page" data-page="${currentPage - 1}"${currentPage <= 1 ? ' disabled' : ''}>上一页</button><button data-cf-action="subsidy-editor-page" data-page="${currentPage + 1}"${currentPage >= totalPages ? ' disabled' : ''}>下一页</button></div></div>`, { footer: '<button class="btn btn-outline" data-cf-action="close-modal">关闭</button>' });
  }

  function subsidyRecordEditModal(ledger, recordId, query = '', page = 1) {
    ...
    state.modal = { type: 'subsidy-record-editor', ledgerId: ledger.id, recordId, query, page };
  ...
  async function handleAction(action, element) {
    ...
    if (action === 'view-subsidy-sheet') return subsidyDetailsModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId), element.dataset.sheet, 1, state.modal?.query);
    if (action === 'view-subsidy-page') return subsidyDetailsModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId), state.modal?.sheet, element.dataset.page, state.modal?.query);
    if (action === 'search-subsidy-sheet') return subsidyDetailsModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId), state.modal?.sheet, 1, document.getElementById('cf-subsidy-query')?.value);
    if (action === 'clear-subsidy-sheet-search') return subsidyDetailsModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId), state.modal?.sheet);
    if (action === 'edit-subsidy') return subsidyRecordListModal(findById('farmlandSubsidyLedgers', element.dataset.id));
    if (action === 'search-subsidy-editor') return subsidyRecordListModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId), 1, document.getElementById('cf-subsidy-editor-query')?.value);
    if (action === 'clear-subsidy-editor-search') return subsidyRecordListModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId));
    if (action === 'subsidy-editor-page') return subsidyRecordListModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId), element.dataset.page, state.modal?.query);
    if (action === 'edit-subsidy-record') return subsidyRecordEditModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId), element.dataset.id, state.modal?.query, state.modal?.page);
    if (action === 'back-subsidy-editor') return subsidyRecordListModal(findById('farmlandSubsidyLedgers', state.modal?.ledgerId), state.modal?.page, state.modal?.query);
