...
(function contractFeeWorkspace(root) {
  ...
  function templatePreviewHtml(batch) {
    ...
    return pages.map((items, pageIndex) => { const rows = items.map((item, index) => `<tr>${rowCells(item, pageIndex * perPage + index).map((cell) => `<td>${escapeHtml(cell || '')}</td>`).join('')}</tr>`).join(''); const isLast = pageIndex === pages.length - 1; return `<article class="cf-print-sheet ${paper}" data-cf-print-page="${pageIndex + 1}"><h1>${escapeHtml(batch.title || template?.title || templateLabel(batch.templateKey))}</h1><div class="cf-print-meta"><span>编制单位：${escapeHtml(batch.villageName || '')}</span><span>期间：${escapeHtml(batch.period || '')}</span><span>日期：${escapeHtml(batch.batchDate || '')}</span><span>单位：元</span></div><table><thead><tr>${headers.map((header) => `<th>${escapeHtml(header)}</th>`).join('')}</tr></thead><tbody>${rows}${isLast ? `<tr><td colspan="${Math.max(1, headers.length - 3)}">合计</td><td colspan="3">${money(total)}　共 ${batch.items.length} 人</td></tr>` : ''}</tbody></table><div class="cf-print-signers"><span>审批人：${escapeHtml(batch.signers?.approver || '')}</span><span>制表人：${escapeHtml(batch.signers?.maker || '')}</span>${batch.templateKey === 'casual_labor' ? `<span>经办人：${escapeHtml(batch.signers?.handler || '')}</span>` : ''}<span>第 ${pageIndex + 1}/${pages.length} 页</span></div></article>`; }).join('');
  }

  function templatePreviewModal(batch) { const perPage = Math.max(1, Number(batch.printSettings?.rowsPerPage || 10)); const pages = Math.max(1, Math.ceil((batch.items || []).length / perPage)); state.modal = { type: 'template-preview', batchId: batch.id }; openModal('打印预览', `<div class="cf-hint">纸张：${escapeHtml(batch.printSettings?.paper || 'A5')}；每页 ${perPage} 人；共 ${pages} 页。系统会自动分页；返回编辑表修改任意内容后，重新预览会即时生成最新分页与金额。</div><div class="cf-row-actions" style="margin:8px 0">${Array.from({ length: pages }, (_value, index) => `<button class="btn btn-outline" data-cf-preview-page="${index + 1}">第 ${index + 1} 页</button>`).join('')}</div><div id="cf-template-preview">${templatePreviewHtml(batch)}</div>`, { footer: `<button class="btn btn-outline" data-cf-action="close-modal">关闭</button>${['draft', 'prepared'].includes(batch.status) ? `<button class="btn btn-outline" data-cf-action="edit-template-batch" data-id="${batch.id}">返回编辑表</button>` : ''}${batch.status === 'draft' ? `<button class="btn btn-outline" data-cf-action="prepare-template" data-id="${batch.id}">准备打印</button>` : ''}<button class="btn btn-outline" data-cf-action="complete-template" data-id="${batch.id}">发放完成并同步</button><button class="btn btn-primary" data-cf-action="print-template">打印</button>` }); }
