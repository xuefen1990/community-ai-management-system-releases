...
(function contractFeeWorkspace(root) {
  ...
  function templatePreviewHtml(batch, { interactive = false } = {}) {
    ...
    const headers = canEdit ? [...baseHeaders, '操作'] : baseHeaders;
  ...
  async function completeTemplateBatch(id) { const current = findById('disbursementBatches', id); const resolutions = Object.fromEntries((current?.items || []).filter((item) => item.personId).map((item) => [item.id, { personId: item.personId, bankCardDecision: 'sync' }])); const plan = model.disbursementResidentSyncPlan(current, state.database.personnel, resolutions); const unresolved = plan.filter((entry) => ['manual', 'missing'].includes(entry.status)); if (unresolved.length) { state.modal = { type: 'template-sync', batchId: id }; return openModal('处理居民关联与银行卡', `<div class="cf-hint">以下资料不能自动同步，请逐条确认居民；卡号不同时可选择仅本次使用或更新居民默认卡。</div><div class="cf-table-wrap"><table class="cf-table"><thead><tr><th>姓名</th><th>组别</th><th>银行卡</th><th>确认居民</th><th>处理方式</th></tr></thead><tbody>${unresolved.map((entry) => `<tr data-sync-item="${escapeHtml(entry.itemId)}"><td>${escapeHtml(entry.item.name)}</td><td>${escapeHtml(entry.item.groupName || '—')}</td><td>${escapeHtml(entry.item.bankCard || '—')}</td><td><select name="personId"><option value="">请选择</option>${(entry.candidates || []).map((candidate) => `<option value="${escapeHtml(candidate.personId)}">${escapeHtml(model.personName(candidate.person))} · ${escapeHtml(model.personGroup(candidate.person) || '未分组')}</option>`).join('')}</select></td><td><select name="decision"><option value="once">仅本次使用</option><option value="sync">同步默认卡</option></select></td></tr>`).join('')}</tbody></table></div>`, { footer: '<button class="btn btn-outline" data-cf-action="close-modal">取消</button><button class="btn btn-primary" data-cf-action="confirm-template-sync">确认并发放完成</button>' }); } const result = model.completeTemplateDisbursementBatch(current, { personnel: state.database.personnel, resolutions }); state.database.personnel = result.personnel; state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1, result.batch); await saveDatabase('已完成发放并同步居民资料'); closeModal(); renderShell(); }
  async function confirmTemplateSync() { const current = findById('disbursementBatches', state.modal?.batchId); const resolutions = {}; document.querySelectorAll('[data-sync-item]').forEach((row) => { resolutions[row.dataset.syncItem] = { personId: row.querySelector('[name="personId"]')?.value, bankCardDecision: row.querySelector('[name="decision"]')?.value }; }); const result = model.completeTemplateDisbursementBatch(current, { personnel: state.database.personnel, resolutions }); state.database.personnel = result.personnel; state.database.disbursementBatches.splice(state.database.disbursementBatches.indexOf(current), 1, result.batch); await saveDatabase('已完成发放并同步居民资料'); closeModal(); renderShell(); }
  ...
  async function handleAction(action, element) {
    ...
    if (action === 'confirm-template-sync') return confirmTemplateSync();
